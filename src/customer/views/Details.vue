...
<template>
  <div class="details" v-if="product">
    ...
    <div class="page-actions">
      <Button variant="outline" size="sm" @click="toggleWish">{{ wish.has(product.id) ? 'Bỏ thích' : 'Yêu thích' }}</Button>
    ...
    <Card class="details-card">
      <div class="layout">
        <div class="gallery" role="group" aria-label="Bộ sưu tập ảnh">
          ...
          <div class="thumbs" role="listbox" aria-label="Ảnh thu nhỏ" @keydown.left.prevent="prevImg" @keydown.right.prevent="nextImg" tabindex="0">
            <button v-for="(img, idx) in gallery" :key="idx" @click="activeIndex = idx" :class="['thumb', { active: activeIndex===idx }]" :aria-selected="activeIndex===idx">
              <img :src="img" alt="" />
            </button>
          </div>
        </div>
        <div class="panel">
          ...
          <fieldset class="options">
            ...
            <div class="field">
              <label>Kích cỡ</label>
              <div class="chips">
                <button v-for="s in sizes" :key="s" :class="['chip', { selected: size===s }]" @click="size=s" :aria-pressed="size===s">{{ s }}</button>
              </div>
            </div>
            <div class="field">
              <label>Màu sắc</label>
              <div class="chips">
                <button v-for="c in colors" :key="c" :class="['chip', { selected: color===c }]" @click="color=c" :aria-pressed="color===c">{{ c }}</button>
              </div>
            </div>
            <div class="field qty">
              <label>Số lượng</label>
              <input type="number" min="1" :max="Math.max(1, product.stock)" v-model.number="quantity" :aria-invalid="quantity<1 || quantity>product.stock" />
            </div>
          </fieldset>
          <div class="actions">
            <button class="wish" :aria-pressed="wish.has(product.id)" @click="toggleWish">{{ wish.has(product.id) ? '❤️' : '🤍' }}</button>
            <Button :disabled="product.stock === 0" @click="addToCart" aria-disabled="product.stock === 0">Thêm vào giỏ</Button>
    ...
    <Card class="mt-2" role="region" aria-label="Thông tin chi tiết">
      ...
      <div v-if="tab==='desc'">{{ product.description }}</div>
    ...
    <Card class="mt-2" role="region" aria-label="Sản phẩm liên quan">
      <template #header><strong>Sản phẩm liên quan</strong></template>
    ...
    <div class="sticky-bar">
      <div class="sticky-price">{{ formatCurrency(product.price) }}</div>
  ...
  <teleport to="body">
    <div v-if="showSizeGuide" class="modal-overlay" @click.self="showSizeGuide=false">
      <div class="modal size-modal">
        <div class="modal-header"><strong>Bảng size tham khảo</strong><button class="modal-close" @click="showSizeGuide=false">×</button></div>
...
const addToCart = () => {
  ...
  cart.add({ id: `${product.value.id}-${size.value}-${color.value}`, name: display, price: product.value.price, image: activeImage.value }, quantity.value)
}
...
const prevImg = () => { activeIndex.value = (activeIndex.value - 1 + gallery.value.length) % gallery.value.length }
const nextImg = () => { activeIndex.value = (activeIndex.value + 1) % gallery.value.length }
...
<style scoped>
.layout { display:grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-lg) }
.gallery { display:flex; flex-direction:column; gap: 8px }
.image { width:100%; border-radius: var(--radius-lg); background: var(--gray-100); object-fit:cover }
.thumbs { display:flex; gap: 8px; flex-wrap:wrap }
.thumb { border:1px solid var(--border); border-radius: var(--radius-md); padding: 2px; background:#fff; cursor:pointer }
.thumb.active { border-color: var(--primary-400) }
.thumb img { width:60px; height:60px; object-fit:cover; border-radius: 6px }
.panel { display:grid; gap: var(--spacing-sm) }
.name { font-size: 1.5rem; font-weight: 800 }
.brand { color: var(--gray-500) }
.price { font-size: 1.25rem; font-weight: 800; color: var(--primary-700) }
.stock.low { color: var(--warning) }
.options { display:grid; gap: var(--spacing-sm) }
.field { display:grid; gap: var(--spacing-xs) }
.chips { display:flex; gap: var(--spacing-sm); flex-wrap:wrap }
...
.field.qty input { width: 96px; padding: var(--form-input-padding); border: var(--form-input-border); border-radius: var(--form-input-radius) }
.mt-2 { margin-top: 0.5rem }
.details-card { padding: var(--spacing-md) }
.tabs { display:flex; gap: 8px }
.tabs button { padding: 6px 10px; border:1px solid var(--border); border-radius: var(--radius-lg); background:#fff; cursor:pointer }
.tabs button.active { background: var(--primary-50); border-color: var(--primary-300); color: var(--primary-800) }
.actions { display:flex; gap: 8px; align-items:center }
.wish { border:1px solid var(--border); background:#fff; border-radius: var(--radius-full); width:36px; height:36px; display:inline-flex; align-items:center; justify-content:center; cursor:pointer }
.page-actions { display:flex; justify-content:flex-end; align-items:center; gap: 8px; margin-bottom: var(--spacing-sm) }
.copied { color: var(--gray-600); font-size: 12px }
.breadcrumbs { display:flex; align-items:center; gap: 8px; color: var(--gray-600); margin-bottom: var(--spacing-sm) }
.breadcrumbs a { text-decoration:none; color: var(--gray-700) }
.breadcrumbs a:hover { color: var(--primary-700) }
/* Size modal reuse modal styles from Catalog */
.size-modal { width: min(520px, 92vw); }
/* Sticky add-to-cart bar for mobile */
.sticky-bar { position: sticky; bottom: 0; display:none; gap: 8px; align-items:center; justify-content:space-between; padding: 8px 12px; border-top:1px solid var(--border); background:#fff; margin-top: var(--spacing-md) }
.sticky-price { font-weight: 800; color: var(--primary-700) }
