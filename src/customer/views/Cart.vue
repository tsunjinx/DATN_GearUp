...
<template>
  <div class="cart">
    <div class="page-actions">
      ...
      <Button variant="outline" size="sm" :disabled="cart.items.length === 0" @click="cart.clear()"> Xoá giỏ </Button>
    </div>
    ...
    <div v-else class="list">
      <div v-for="item in cart.items" :key="item.id" class="row">
        ...
        <input
          ...
          @input="e => cart.updateQuantity(item.id, e.target.value)"
        />
        <div class="price">
      ...
      <div class="coupon-row">
        <input v-model="coupon" class="coupon" placeholder="Mã giảm giá" aria-label="Nhập mã giảm giá" />
        <button class="apply" @click="applyCoupon">Áp dụng</button>
        <span v-if="couponMsg" class="coupon-msg">{{ couponMsg }}</span>
      </div>
      <div class="summary">
        <div>
...
const applyCoupon = async () => {
  ...
  if (res?.data?.valid) {
    discountRate.value = res.data.discount
    couponMsg.value = 'Áp dụng thành công'
  } else {
    discountRate.value = 0
    couponMsg.value = 'Mã không hợp lệ'
  }
}
...
.row {
  display: grid;
  grid-template-columns: 64px 1fr 100px 140px 40px;
  gap: var(--spacing-sm);
  align-items: center;
